...
const goToThePreviousSlide = () => {
    ...
    if (shouldFallback) lastSlide.style.transform = `translateX(${slidesCount * -100}%)`;

    changeSlide(currentSlideIndex - 1, shouldFallback, slidesCount - 1, lastSlide);
};
...
const goToTheNextSlide = () => {
    ...
    if (shouldFallback) firstSlide.style.transform = `translateX(${slidesCount * 100}%)`;
...
const changeSlide = (index, shouldFallback, fallbackIndex, fallbackSlide) => {
    ...
    const animation = slides.animate(
        [{transform: `translateX(${currentSlideIndex * -100}%)`}, {transform: `translateX(${index * -100}%)`}],
        {
    ...
    animation.addEventListener('finish', () => {
        if (shouldFallback) {
            ...
            slides.animate([{transform: `translateX(${fallbackIndex * -100}%)`}], {fill: 'forwards'});
